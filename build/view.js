...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const filterUsers = e => {
    ...
    document.querySelectorAll('.ob-user-grid-filters > button').forEach(btn => btn.classList.remove('active'));
    ...
    if (e.target.dataset.category !== 'ALL') {
      ...
      const obUserElemFiltered = document.querySelectorAll(`.ob-user-grid-elem:not(:has([data-category="${e.target.dataset.category}"]))`);
  ...
  arrowNext.addEventListener('click', () => {
    const userElem = document.querySelectorAll('.ob-user-grid-elem:not(.hide)');
    usersContainer.scrollTo({
      left: usersContainer.scrollLeft + userElem[0].offsetWidth + 10,
      behavior: 'smooth'
    });
    arrowPrev.classList.remove('hide');
    if (usersContainer.offsetWidth + usersContainer.scrollLeft + 5 >= usersContainer.scrollWidth) arrowNext.classList.add('hide');
  });
  ...
  usersContainer.addEventListener('scroll', () => {
    if (usersContainer.offsetWidth + usersContainer.scrollLeft + 5 >= usersContainer.scrollWidth) {
      arrowNext.classList.add('hide');
      arrowPrev.classList.remove('hide');
